    ...
    async toArray<T>(): Promise<T[]> {
        return await (await match(getSessionSecret())
            ...
            .with({ tag: "some" }, async (x) => {
                ...
                let collectionUrl = new URL(this.collectionUrl)
                collectionUrl.pathname = collectionUrl.pathname + "/find/toarray"
    ...
    async count(countOptions?: CountOptions): Promise<number> {
        return await (await match(getSessionSecret())
            ...
            .with({ tag: "some" }, async (x) => {
                ...
                let collectionUrl = new URL(this.collectionUrl)
                collectionUrl.pathname = collectionUrl.pathname + "/find/count"

                const result = await fetchPostEncrypted(collectionUrl.toString(), { filter: this.filter, options: this.options, countOptions: countOptions }, x.value)
